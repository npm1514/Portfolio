app.controller("earthCtrl", function($scope, $window, $interval, $rootScope ) {
    ...
    $rootScope.makearc = function () {
      ...
      $rootScope.duringupdate.forEach(function(a) {
        for (var b = 0; b < a.people.length; b++) {
          if (a.people[b].geometryfrom.coordinates) {
            linkcamp.push({
              "source": a.people[b].geometryfrom.coordinates,
              "target": a.people[b].geometrycamp.coordinates
            });
          }
        }
      });
      ...
      $rootScope.afterupdate.forEach(function(a) {
        for (var b = 0; b < a.people.length; b++) {
          if (a.people[b].geometryto.coordinates) {
            linkjob.push({
              "source": a.people[b].geometrycamp.coordinates,
              "target": a.people[b].geometryto.coordinates
            });
          }
        }
      });
      ...
      if (linkcamp) {
        svg.append("g")
          ...
          .attr("d", function(d) {
            ...
            console.log("I'm a nicer error message!");
            console.log(err);
          }
          });
      }
